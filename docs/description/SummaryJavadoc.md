...
a non-empty `{@return}` are allowed. Check also violate Javadoc that
does not contain first sentence, though with `{@return}` a period is not
required as the Javadoc tool adds it.
